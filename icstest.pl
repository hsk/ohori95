...
%  test(c_kapp) :- 'C'(λ(x::u,x)!int).
  test(c_record) :- 'C'([x=1,y=2]).
  test(c_record) :- 'C'([x=1,y=2]#x).
  test(c_record) :- 'C'(modify([x=1,y=2],x,2)).
  test(c_variant) :- 'C'({[eint=1]}).
  test(c_variant) :- 'C'(case({[eint=1]},{[eint=λ(x,x),eadd=λ(x,add$x#'1'$x#'2')]})),!.
