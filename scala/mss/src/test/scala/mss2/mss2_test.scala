...
class mss2_test extends FunSpec {
  ...
  describe("typing") {
    ...
    it("let4") { test(
      ...
      "let id: ∀x0::U.∀x1::U.x0->x1->x0=Poly(λx:x0.λy:x1.x : ∀x0::U.∀x1::U.x0->x1->x0) in (id!int!int) 1 2",
      "int") }

    it("let_poly") { tesk(
      "let id = λx.x#a in id",
      "let id: ∀x1::U.∀x2::{{a:x1}}.x2->x1=Poly(λx:x2.x:x2#a : ∀x1::U.∀x2::{{a:x1}}.x2->x1) in id!x3!x4",
      "∀x4::{{a:x3}}.∀x3::U.x4->x3",
